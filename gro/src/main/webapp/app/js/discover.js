...
'2000', '2001', '2002', '2003', '2004', '2005', '2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016'];
	...
	$scope.submitStrongIntrest=function(){
	    var user = $window.localStorage.getItem('loggedInUser');
	    user = jQuery.parseJSON(user);
	    $scope.feedback.name=user.fullName;
	    $scope.feedback.email=user.user;
	...
	console.log('toShow is '+toShow);
		if(toShow){
			...
			$('html,body').animate({
			    scrollTop: $('#discover_steps').offset().top
			}, 1000);
			$scope.show_strongintrest_steps(false);
		}
		else{
	...
	$scope.show_strongintrest_steps = function(toShow){
	    console.log('toShow is '+toShow);
	    if(toShow){
	        document.getElementById('strongintrest_steps').style.display = "";
	        document.getElementById('results').style.display = "none";
	        $scope.show_discover_steps(false);
	        $('html,body').animate({
	            scrollTop: $('#strongintrest_steps').offset().top
	        }, 1000);
	    }
	    else{
	        document.getElementById('strongintrest_steps').style.display = "none";
	        document.getElementById('results').style.display = "none";
	    }
	}
